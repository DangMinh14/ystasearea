...
const handleKeydown = (event: KeyboardEvent) => {
  ...
  if (event.key === 'ArrowUp') desired = { x: 0, y: -1 }
  if (event.key === 'ArrowDown') desired = { x: 0, y: 1 }
  if (event.key === 'ArrowLeft') desired = { x: -1, y: 0 }
  if (event.key === 'ArrowRight') desired = { x: 1, y: 0 }
...
const draw = () => {
  ...
  if (specialItem) {
    ...
    ctx.beginPath()
    ctx.arc((specialItem.position.x + 0.5) * cellSize, (specialItem.position.y + 0.5) * cellSize, cellSize * 0.32, 0, Math.PI * 2)
    ctx.fill()
  }
    ...
    <header class="snake__header">
      <div>
        ...
        <div class="snake__modes" role="group" aria-label="Game mode">
          <button class="snake__mode" :class="{ 'snake__mode--active': mode === 'walls' }" type="button" @click="setMode('walls')">
            Có tường
          </button>
          <button class="snake__mode" :class="{ 'snake__mode--active': mode === 'wrap' }" type="button" @click="setMode('wrap')">
            Không có tường
          </button>
        </div>
      </div>
      <div class="snake__controls">
        ...
        <button class="snake__button" type="button" @click="togglePause">{{ isPaused ? 'Tiếp tục' : 'Tạm dừng' }}</button>
        <button class="snake__button snake__button--ghost" type="button" @click="restart">Chơi lại</button>
      </div>
    </header>
